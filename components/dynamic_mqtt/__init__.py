...
DynamicMqttMessageTrigger = dynamic_mqtt_ns.class_(
    "DynamicMqttMessageTrigger", automation.Trigger.template(cg.std_string, cg.std_string)
)
...
async def to_code(config):
    ...
    # triggers
    for m in config[CONF_ON_MESSAGE]:
        trig = cg.new_Pvariable(m["then"][CONF_ID], var)
        cg.add(trig.set_topic(m[CONF_TOPIC]))
        await automation.build_automation(trig, [(cg.std_string, "topic"), (cg.std_string, "payload")], m["then"])
